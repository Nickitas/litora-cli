...
На этапе EXPORT-02 `profiles.csv` сохраняет явно переданные трассы. `Profile`
содержит ID, название и упорядоченные `NodeIDs`; все ID проверяются до создания
файлов. Таблица хранит `point_index` от 1, накопленное `distance_m` по X/Y LAEA,
координаты, отметку, глубину, способ выборки и качество каждого узла.

VIEW-03 реализует автоматический выбор содержательных трасс «берег →
глубоководная часть». Команда `lito seabed render` воспроизводимо выбирает
западный, восточный и северный береговые узлы, прокладывает путь к ближайшему
глубоководному ядру по фактическим рёбрам и обновляет тот же `profiles.csv`.
Вес пути штрафует повторный выход на берег и уменьшение глубины; случайные
последовательности узлов не используются.
