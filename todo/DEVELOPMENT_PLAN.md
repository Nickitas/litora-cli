...
- Размер: M.
- Работы:
  - реализовать обратную сферическую LAEA-проекцию;
  - сохранять `lat/lon` каждого узла вместе с `X/Y`;
  - добавить тесты прямого и обратного преобразования на границах Чёрного моря.
- Готово, когда:
  - ошибка цикла `WGS 84 → LAEA → WGS 84` документирована и существенно меньше
    размера минимальной ячейки;
  - тесты включают западную, восточную, северную и южную границы акватории.
