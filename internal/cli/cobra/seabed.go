...
Подкоманда build превращает плоскую full-quad MSH и подтверждённую батиметрию
в MSH/VTU/CSV/JSON модели дна. Render повторно строит обзорную карту,
увеличенные фрагменты, 3D-рельеф и профили без пересчёта глубин. Adapt и
generate-adaptive создают поле требуемого размера и фактическую адаптивную
full-quad сетку из принятого MSH lito-seabed/v1; compare-adaptive сравнивает
три генератора на общих контрольных полях. Validate выполняет научную проверку
сохранения рельефа по отдельной опорной модели. QA-03 одной командой проверяет
полный контур 1000 м, экспортирует модель и фиксирует топологию, интегральные
ориентиры и ресурсы.`,
}
