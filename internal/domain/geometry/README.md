...
- [Одномерная модель CERC](#одномерная-модель-cerc)
...
## Одномерная модель CERC

`RunLongshoreCERC` — основной расчётный API для инженерного моделирования
вдольберегового переноса. В отличие от исторической функции
`SimulateWaveErosion`, он принимает не эвристические силу и ветер, а
обязательный фактический ряд `WaveClimate` с `Hs`, `Tp`, направлением и
длительностью каждого состояния. Функция требует `BathymetryGrid` и возвращает
пространственные объёмы ячеек в `м³`, поток `м³/с`, снимки линии и невязку
баланса.

В `LongshoreModelResult.InputQuality` фиксируются качество реального волнового
ряда и применённой батиметрии: длительность, временные пропуски, разрешение
сетки, число интерполированных и ближайших выборок глубины. Поля
`LeftBoundaryTransportM3S` и `RightBoundaryTransportM3S` конфигурации задают
измеренные внешние потоки на торцах; нулевые значения означают закрытые
границы. `LongshoreSedimentSource` задаёт документированный внешний приток или
сток в конкретной ячейке и включается в замыкание баланса; он не описывает
исчерпание запаса активного слоя. Функция `CalibrateCERCCoefficient` подбирает только коэффициент CERC
по независимым годовым скоростям изменения береговой линии и требует минимум
репрезентативного года волновых данных.

`LongshoreStructure` определяет коэффициент пропуска потока на грани двух
ячеек. Он сохраняет замыкание объёма при перехвате транспорта сооружением, но
не является моделью волновой дифракции, отражения или пространственного
обтекания.

Расчёт включает дисперсию, групповую скорость, рефракцию по закону Снеллиуса,
shoaling, критерий разрушения `Hb = γhb`, CERC-поток и уравнение
неразрывности. Он применим только к незамкнутому сегменту берега с активным
профилем, который допускает one-line представление. Полное описание формата,
формул и ограничений находится в [`docs/cerc-one-line-model.md`](../../../docs/cerc-one-line-model.md).

Загруженный ряд может содержать наблюдения, реанализ или прогноз. Его наличие
само по себе не подтверждает исследовательскую пригодность сценария.

`LoadWaveClimate` принимает собственный JSON/CSV-формат Lito, а также
исходный почасовой JSON Open-Meteo Marine. Для внешнего ответа источник всегда
передаётся отдельной строкой и включается в `LongshoreModelResult`. Результат
также содержит `ScenarioClassification`: `demo` для технического примера или
`unclassified`, когда исследовательская пригодность не подтверждена. Успех
численного расчёта не меняет эту классификацию.

Исторические `SimulateWaveErosion*` и `CalculateSedimentTransport*` сохранены
для обратной совместимости и экспериментальных сценариев; они не являются
расчётным ядром CLI и не должны интерпретироваться как CERC/Komar-модель.
...
Батиметрические данные интерполируются билинейным методом. Метод
`SampleDepthDetailed` различает точное совпадение, билинейную интерполяцию и
ближайшую замену, а также возвращает расстояние до использованной исходной
точки. Для узлов расчётной сетки согласование с береговым нулём выполняет
отдельный пакет `internal/domain/seabed`; NoData не заменяется нулём.
...
| Вероятность | Ожидаемый интервал при шаге 1 год | Условный уровень воздействия |
|-------------|-----------------|-----------------|
| 0.0 | События отключены | Нулевой |
| 0.05 | 20 шагов | Низкий |
| 0.1 | 10 шагов | Средний |
| 0.2+ | 5 шагов или меньше | Высокий |

Это независимые испытания Бернулли на каждом шаге, а не откалиброванное
распределение штормов. Модель не воспроизводит длительность, кластеризацию или
совместное распределение характеристик волнения.
...
**Примеры параметрических значений:**

| Уровень воздействия | Подъём (м/год) | Через 50 модельных лет |
|----------|----------------|--------------|
| Низкий | 0.003 | +0.15 м |
| Средний | 0.006 | +0.30 м |
| Высокий | 0.010 | +0.50 м |

Значения задаются вручную для анализа чувствительности. Они не являются
траекториями RCP/SSP, климатическим прогнозом или оценкой для конкретного
календарного года. Вероятность шторма в этой эвристике также не заменяет
распределение штормов, полученное из наблюдений или климатических расчётов.
...
Формат JSON — массив точек с `{lat, lon, depth}`. Загрузчик принимает только
координаты области Чёрного моря `40,5–47,5° с. ш., 27,0–42,5° в. д.`, глубины
от `−3000` до `0` м и автоматически строит регулярную сетку с заданным
разрешением. Прямоугольная проверка не заменяет обязательную маску полигона
Marine Regions MRGID 3319 при подготовке производного набора.
...
`BuildGrid` применяет тот же числовой и физический контракт, что и загрузчик:
координаты, глубины и разрешение должны быть конечными, отметка дна находится
в диапазоне `−3000…0 м`. Отсутствие хотя бы одного из четырёх соседей не
превращается в нулевую глубину: строгая интерполяция возвращает ошибку, а
`SampleDepthDetailed` использует ближайшую реальную точку только при явно
заданном допустимом радиусе и сообщает расстояние до неё.

### Контроль качества интерполяции

QA-01 закрепляет поведение на аналитических поверхностях: плоскость должна
восстанавливаться точно, чаша — сохранять симметрию и монотонность, ступень —
не давать выбросов, а NoData — оставаться явным. Property-тест и Go fuzzing
дополнительно проверяют, что билинейный результат конечен, не меняет знак и
лежит между минимумом и максимумом четырёх исходных узлов. Береговое условие
`Z = 0` проверяется интеграционными тестами пакета `seabed`.

Полная матрица сценариев и команды запуска приведены в
...
- **depth > 0** — надводная высота, отклоняется загрузчиком батиметрии
...
### Временная динамика с параметрическим усилением воздействия
...
func main() {
    ...
    temporalParams := geometry.TemporalParameters{
        ...
        SeaLevelRise:         0.006, // вручную заданные 6 мм/год
