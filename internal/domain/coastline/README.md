...
Локальный файл эталонного участка из `data/benchmarks/` также может быть
передан в `Load` и CLI через `--input`. Если JSON содержит поле `coastline`,
модуль извлекает из него последовательность `LatLon`; это позволяет запускать
модель на проверенном локальном сегменте без ручного копирования координат.
